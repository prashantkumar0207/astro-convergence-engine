...
content-hash pinned); B dense sweep - WIRING/COVERAGE CHECK, not correctness
evidence (both sides call the identical kp_chain() function; see the gate's
own "disclosure" field); C independent validator
(validate_kp_significator_holdout.py, a from-scratch reimplementation,
importing nothing from engine.kp.significators, checked against static
expected data since the 2026-08-27 repair - see that file's own docstring);
D non-invasiveness (module-identity/table-integrity check, NOT a correctness
check - confirms engine.kp.significators is correctly importable and
content-hash matches the certified pinned value, and that KP_CHAIN_V1,
PARASHARI_DRISHTI_V1, sign_lord.py, and the KP_KRISHNAMURTI profile remain
untouched, and that no Parashari aspect code is imported anywhere in the
production module, the certifier, or the validator; it cannot see a wrong
value computed correctly-looking code, only an altered import path or
mutated frozen constant); E boundary cases - WIRING/COVERAGE CHECK for its
sub-lord portion, genuine correctness check for its sign_of() portion (see
the gate's own "disclosure" field); F retrograde cases; G node/aspect cases;
H strength-order cases; I protected holdout - PRIMARY CORRECTNESS EVIDENCE
(production output compared against STATIC expected values frozen from the
independent validator's own from-scratch logic, never regenerated by
production code at certification time - see HOLDOUT's own module comment);
J negative controls (real planted mutations against local copies, never the
production singleton itself, confirmed detected).

**Certification-integrity repair (2026-08-27, "CEO DIRECTIVE - KP_SIGNIFICATOR_V1
CERTIFICATION REPAIR", see docs/decisions/DP-030-kp-significator-v1-
remediation-readiness.md and the corresponding docs/DECISION_LOG.md entry):**
an external forensic mutation experiment (2026-08-26) proved every gate in
the PRIOR version of this file reported PASS while the production module's
computed sub-lord was wrong at every one of 3600 sampled longitudes, and
gate I's own "independent" reference imported and called
engine.kp.significators.signification_set() directly - a genuine
certification-integrity defect, independently reproduced during this repair
by live monkeypatch corruption (see gate return values' own "disclosure"
fields above for what changed). scripts/check_mutation_detection.py is the
standing, hermetic, CI-suitable regression gate against this defect class -
it corrupts engine.kp.significators._signifies() (dropping the "owner"
membership clause, which changes neither the frozen constants nor the
content hash) and requires this certifier's own gates to detect it.

No computational oracle exists for KP significators (DP-028 section D,
DP-029 section 4, reaffirmed) - gate I now carries the primary evidentiary
weight (production vs. static, independently-derived expected values); gate
C is corroborating (a second, differently-authored implementation's own
internal checks); gates B/E/D are wiring/coverage/table-integrity checks,
disclosed as such, not correctness evidence; gates F/G/H/J exercise specific
structural correctness properties (retrograde disclosure independence,
node-substitution priority, significator-category recognition, and three
planted-mutation detections) directly.
...
def gate_b_dense_sweep():
    ...
    return {"points": points, "mismatches": 0,
            "scope": "7th cusp longitude sweep, production wiring vs. a second call to the same "
                     "kp_chain() function used internally by chart construction",
            "classification": "wiring_coverage_not_correctness_evidence",
            "disclosure": "both sides call the identical kp_chain() function; this proves the "
                           "production module correctly WIRES that function's output into its own "
                           "chart data structures across a dense sample, not that kp_chain()'s "
                           "own values are correct (KP_CHAIN_V1 already certifies that separately "
                           "against PyJHora) or that the significator-derivation logic downstream "
                           "is correct (see gate I for that evidence). A prior version of this gate "
                           "was presented as correctness evidence; corrected 2026-08-27 per the "
                           "certification-integrity repair, see docs/decisions/DP-030."}


def gate_c_independent_validator():
    """Runs validate_kp_significator_holdout.py as a SEPARATE PROCESS (so it
    cannot share any monkeypatched or corrupted in-process state with this
    certifier). Since the 2026-08-27 repair, that file's own battery
    includes a genuine correctness check (its own from-scratch judge()
    output compared against the same static expected values gate I uses,
    on the same twelve real charts) in addition to its pre-existing dense
    sweep and structural-invariant checks. Corroborating, not primary:
    gate I is the primary correctness evidence (see its own docstring)."""
    ...
    return {"result": "PASS",
            ...
            "note": "separate-process reimplementation; since 2026-08-27 also checked against "
                    "static expected data (see validate_kp_significator_holdout.py)"}
...
def gate_e_boundary_cases():
    ...
    return {"cases_checked": cases_checked, "mismatches": 0,
            ...
            "classification": "wiring_coverage_not_correctness_evidence",
            "disclosure": "the sub-lord portion of this gate calls the same kp_chain() function on "
                           "both sides (see gate B's own disclosure); only the sign_of() boundary "
                           "sweep is a genuine second, differently-implemented computation. A prior "
                           "version of this gate was presented as correctness evidence; corrected "
                           "2026-08-27, see docs/decisions/DP-030."}
...
def gate_g_node_aspect_cases():
    ...
    if substitute != "Jupiter":
        mismatches += 1
    # Wiring check, not independent correctness evidence (both calls go
    # through the same production signification_set()/_signifies() path,
    # so a shared corruption there would not be caught here - confirmed by
    # scripts/check_mutation_detection.py kp_significator_logic, which is
    # blind on this gate for exactly that reason). It does verify that
    # judge_marriage's own internal call and a direct call agree on the
    # SAME target planet, catching a wiring mistake between the two.
...
#: STATIC expected values (KP_SIGNIFICATOR_V1 CERTIFICATION REPAIR, 2026-08-27).
#:
#: Each `expected_*` field was generated ONCE, offline, by calling ONLY
#: validate_kp_significator_holdout.py's own from-scratch judge() function
#: (which imports nothing from engine.kp.significators) against these same
#: twelve real charts - never by calling judge_marriage() or any other
#: production function. This replaces the prior _independent_judge_marriage_
#: reference() design, which imported and called engine.kp.significators.
#: signification_set() directly: a genuine certification-integrity defect
#: (agreement reduced to x == x) found by an external forensic mutation
#: experiment (2026-08-26, see docs/decisions/DP-030-kp-significator-v1-
#: remediation-readiness.md) and confirmed independently in this repair by
#: live monkeypatch corruption of kp_chain: the prior gate I reported PASS
#: even though the corrupted run's verdict distribution differed from the
#: pristine run's (MIXED:6/DENIED:3/PROMISED:3 -> MIXED:7/DENIED:3/
#: PROMISED:2).
#:
#: No third-party computational oracle exists for this layer (DP-028
#: section D, DP-029 section 4, reaffirmed) - these static values are
#: cross-implementation agreement evidence (two independently-authored
#: implementations converging on the same real-chart results), not
#: external oracle validation. Disclosed explicitly in the certification
#: artifact's own "oracle" field.
HOLDOUT = [
...
def gate_i_protected_holdout():
    """Compares LIVE production output (judge_marriage) against the STATIC
    expected_* fields frozen into HOLDOUT above - never against a value
    re-derived at certification time by calling any engine.kp.significators
    function. This is the fix for the certification-integrity defect: the
    prior design's "independent" reference called signification_set()
    directly from the module under test, so both sides moved together and
    every comparison reduced to x == x (confirmed by live corruption during
    this repair)."""
    ...
    for case in HOLDOUT:
        ...
        if production_as_dict != expected:
            fail(f"holdout mismatch on {case['id']}: production={production_as_dict} expected(static)={expected}")
    return {"cases": len(HOLDOUT), "verdict_distribution": verdict_counts,
            "methodology": "production output compared against STATIC expected values frozen "
                            "from validate_kp_significator_holdout.py's own from-scratch "
                            "reimplementation (never regenerated by production code at "
                            "certification time); real ephemeris-driven charts, independent of "
                            "every deliberately-constructed synthetic case in gates E-H, never "
                            "used to tune any rule",
            "expected_value_provenance": "validate_kp_significator_holdout.py judge(), frozen 2026-08-27",
...
def main():
    ...
    report = {
        ...
        "oracle": {"package": None, "note": "no third-party computational oracle exists for KP "
                                             "significators (DP-028 section D, DP-029 section 4); "
                                             "certification rests on cross-implementation agreement "
                                             "between engine.kp.significators (production) and "
                                             "validate_kp_significator_holdout.py (an independently "
                                             "authored, from-scratch reimplementation) - gate I "
                                             "compares production against STATIC values frozen from "
                                             "the independent reimplementation's own output, and "
                                             "gate C runs that reimplementation's own battery "
                                             "standalone. This is agreement between two "
                                             "independently-written implementations of the same "
                                             "frozen rule, not external oracle validation, and is "
                                             "not evidence that the frozen rule itself matches "
                                             "primary KP doctrine (see docs/decisions/DP-030 section "
                                             "4 for that separate, still-open question)."},
        "certification_integrity_repair": {
            "date": "2026-08-27",
            "authorization": "CEO DIRECTIVE - KP_SIGNIFICATOR_V1 CERTIFICATION REPAIR",
            "defect_found": "an external forensic mutation experiment (2026-08-26) proved every "
                             "gate in the prior version of this file reported PASS while the "
                             "production module's computed sub-lord was wrong at 3600/3600 sampled "
                             "longitudes; gate I's own 'independent' reference imported and called "
                             "engine.kp.significators.signification_set() directly (x == x); the "
                             "HOLDOUT list carried no expected values of any kind; gates B/E "
                             "compared kp_chain() against itself and were presented as correctness "
                             "evidence rather than wiring checks",
            "reproduced_independently_this_repair": True,
            "repair_summary": "gate I rewritten to compare production against STATIC expected "
                               "values frozen from validate_kp_significator_holdout.py's own "
                               "from-scratch logic; gates B/E honestly reclassified as wiring/"
                               "coverage checks (see their own 'classification'/'disclosure' "
                               "fields); scripts/check_mutation_detection.py extended with a "
                               "hermetic gate that corrupts engine.kp.significators._signifies() "
                               "directly and requires detection",
            "record": "docs/decisions/DP-030-kp-significator-v1-remediation-readiness.md; "
                       "docs/DECISION_LOG.md certification-integrity-correction sub-entry under "
                       "ADR-0078",
        },
        "gates": {
        ...
        "explicit_non_claims": [
            ...
            "no third-party computational oracle corroborates this design - cross-implementation "
            "agreement (production vs. an independently-authored reimplementation) is the sole "
            "evidentiary basis, disclosed as such and not overstated as oracle validation",
