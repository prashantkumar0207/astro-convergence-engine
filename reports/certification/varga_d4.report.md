...
- oracle execution is conditional: a GENUINE PyJHora comparison when the library is importable (CI's hash-pinned oracle job, into which certify_d4.py is now wired), disclosure-only otherwise. The gates block records which of the two actually happened - a disclosure run is never presented as oracle verification
