...
| Field | Value |
|---|---|
| Status | ACTIVE REGISTER. Twenty-seven questions. **Owner ratifications of 2026-08-13: Q13 resolved; Q1 and Q8 ratified in part (ADR-0033); ADR-0005 ratified, making Tier-0 FORMALLY LOCKED (ADR-0034); ADR-0009/0010/0011 ratified, making the registry vargas owner-ratified CERTIFIED but NOT Locked (ADR-0035); ADR-0012 ratified, closing the varga sign-convention asymmetry (ADR-0036).** **Owner ratification of 2026-08-17: Q8 fully RESOLVED (ADR-0048).** **Owner ratification of 2026-08-17 (later batch): Q1 fully RESOLVED (ADR-0051 D4, accepting DP-001 Option B) - docs/engine/release-owner roles interim-designated to the Project Owner pending explicit delegation; no longer "ratified in part".** **Status enumeration, reconciled 2026-09-18.** **Eleven fully open** (Q2, Q3, Q4, Q5, Q7, Q10, Q12, Q23, Q25, Q26, Q27); **two resolved in part** (Q22, Q24); **six with candidate resolutions awaiting a ratification decision** (Q6, Q9, Q11, Q14, Q15, Q16); **eight resolved** (Q1, Q8, Q13, Q17, Q18, Q19, Q20, Q21). Total 27. Ratification is per-entry; the rest are not ratified merely because Q1 and Q8 are. *Append-only correction of the historical enumeration, recorded rather than silently overwritten: the previous text said "Eleven fully open (Q2, Q3, Q4, Q5, Q7, Q10, Q12, Q22, Q23, Q24, Q25) … three resolved (Q13, Q8, Q1)". Three inaccuracies are corrected. (a) Q22 and Q24 were listed as fully open; both became RESOLVED IN PART on 2026-09-18 under `ADR-0097`'s M1 remediation. (b) Q26 and Q27, created the same day by the authorized Option-C split, were absent from the enumeration entirely. (c) "three resolved" had been inaccurate since 2026-08-14 independently of M2: Q17 and Q18 were resolved by `ADR-0039`, Q19 by `ADR-0040`, Q20 by `ADR-0041` and Q21 by `ADR-0042`, none of which the enumeration absorbed. No question's status is changed by this correction; only the enumeration is brought into agreement with the rows it counts.*|
| Version | 3.0.0 |
| Owner | TBD (see docs/OPEN_QUESTIONS.md Q1) |
| Last updated | 2026-09-18 (**M2, Option-C split.** Version raised to 3.0.0; **the basis for document version numbering is an unresolved governance issue** - `docs/DOCUMENTATION_STANDARD.md` s2 item 4 describes MAJOR/MINOR/PATCH, but that document is a DRAFT placeholder, so no version decision here rests on it. 3.0.0 is a provisional documentation-record update, recording that statements in this register changed meaning. It invokes no precedent in this document, which has no change-history table, and it is open to correction once version governance is settled. Changes: `Q22` and `Q24` move from OPEN to **RESOLVED IN PART** on `ADR-0097`'s M1 remediation, their superseded factual premises are annotated, and their unremediated remainders are split to the new `Q26` (lossless `_render`, the H-7b remainder) and `Q27` (the coordinated multi-record trust-anchor residual). Register count corrected twenty-five -> twenty-seven. No other question's status changed.) |
| Review cadence | TBD |
...
| Q10 | CI provider and mandatory pipeline stages (validation standard is CI-ready but no provider is chosen)? *(Premise corrected 2026-09-05, `ADR-0093`: "no provider is chosen" is factually superseded. GitHub Actions is in use - `.github/workflows/ci.yml`, workflow `engine-ci`, three jobs: `hermetic` (no-oracle gate, run on a two-version Python matrix), `oracle` (PyJHora, hash-pinned) and `governance` (identifier families, document structure). Green runs of that workflow are cited as ratifying evidence throughout `docs/DECISION_LOG.md`. The premise is annotated, not the status: **declaring this question RESOLVED is an owner act and has not been taken**, and what remains genuinely open is whether the pipeline stages now in use are ratified as the answer.)* | Automation of the gates | tools/, repository settings | OPEN |
...
| Q22 | The evidence-agreement gate compares each report against the same `certification_support._render` that produced it. Should an independent check exist? | It proves `report == _render(artifact)`, which is real and catches hand-editing and stale regeneration. It cannot detect a defect **inside** `_render`, where both evidence files would agree on a wrong number. Under the charter's anti-circularity rule this is the analogue of Q14 for that gate. | Evidential strength of Constitution s12 condition 3 | **RESOLVED IN PART, 2026-09-18** (`ADR-0097`, M1/H-7a). *Premise annotated: "closing it needs new verification design, not remediation" was correct, and M1 produced that design.* `engine/tests/test_load_bearing_field_coverage.py` reads report text and artifact JSON directly and never calls or imports `_render`; an AST walk over its own source enforces that independence. Negative-controlled by mutating `_render` itself in a disposable worktree: `drop_mismatches` (19/22 reports changed) and `freeze_comparisons` (10/22) each PASSED the old circular gate and FAILED the new one. **NOT fully resolved:** `_render` is lossy - 44.9% of artifact leaf values overall, 17.0% Tier-0, 11.5% `TRANSIT_V1` - and 39 load-bearing values remain genuinely absent from their reports, frozen in `DECLARED_RENDER_GAPS` and asserted in both directions so the set cannot grow silently. That remainder (H-7b) is split to **Q26**. |
...
| Q24 | Should the `swetest` reference binary and `CHECKSUMS.sha256` itself be integrity-verified? | The binary's SHA-256 is recorded in `certification/ORACLE_ENVIRONMENT.json` and **read by nothing**; what is checked is a version string, which any binary can print. `CHECKSUMS.sha256` is itself unverified, so a coordinated edit of manifest plus assets passes both mechanisms. **The D-001 numerical authority is the one asset in the gate with recorded-but-unverified integrity.** | Integrity of the independent numerical authority; ADR-0032's phrasing that a version check "proves the ORACLE is the pinned build" | **RESOLVED IN PART, 2026-09-18** (`ADR-0097`, M1/B-2). *Premise corrected: the question's own statement that the binary's SHA-256 is "read by nothing" was superseded by commit `12d3b5c`; it is now read on every certifier run.* `certification_support.verify_trust_records()` enforces four checks inside `preflight()`: the `swetest` bytes hash to the recorded digest; `CHECKSUMS.sha256`'s bytes hash to the digest recorded for it; the manifest's git blob identity equals a pinned constant, never recomputed from the current file; and the manifest's per-file digests agree with `ORACLE_ENVIRONMENT.json`'s duplicate list in both directions, so a single-file edit fails. Eight committed negative controls, including a cross-platform CRLF/LF control. **NOT fully resolved:** a coordinated single-commit edit of every repository-held trust record cannot be prevented by any in-repository anchor. That residual is split to **Q27**. |
| Q25 | The only remote-CI evidence in the repository attests the **two-job** workflow at `a460ba2`. No run record exists for the three-job workflow, and this batch changed the workflow again. Should a fresh remote CI run be recorded before any further ratification of ADR-0018? *(Premise corrected 2026-09-05, `ADR-0093`: both factual premises are superseded. The workflow is now three jobs (`hermetic`, `oracle`, `governance`), and green run records for it exist and are cited with run IDs throughout `docs/DECISION_LOG.md` - for example runs `32825473827` and `33061298693` on `main`, and run `33755665778` on PR #13, each reported green across all four check entries. The row's strongest claim, "the repository cannot prove CI passes at HEAD", is therefore no longer true of the workflow generally, though it remains true of any specific unpushed commit. The premise is annotated, not the status: **declaring this question RESOLVED is an owner act and has not been taken**, and `ADR-0018`'s own `CEO_REPORTED` addendum with null `run_url`/`run_id` is unchanged by this correction.)* | ADR-0018's addendum is self-labelled `CEO_REPORTED` with `run_url`, `run_id`, `runner_image` and `pip_version` all null, which the repository commendably flags. What is not flagged is that the attested workflow file no longer exists. **The repository cannot prove CI passes at HEAD, and nothing in this batch claims it does.** | Evidential basis of ADR-0018; whether the governance job has ever run green remotely | OPEN, raised 2026-08-13. Audit NOT-DETERMINABLE section. **Only the owner can produce this evidence: it requires a run on GitHub.** |
| Q26 | Should `certification_support._render` be made lossless, so that every load-bearing artifact value appears in its human-readable report (the H-7b remainder of `Q22`)? | `_render` is a summary, not a serialisation: measured at `a62a2c8` it carries 44.9% of artifact leaf values, 17.0% for Tier-0 and 11.5% for `TRANSIT_V1`. 39 load-bearing values are genuinely absent from their reports. The dominant cause is that `_render` never emits the top-level `oracle` block, so **no certification report states which external oracle or which version produced its comparison**; a second cause is that the gate-rendering comprehension silently drops any value longer than 80 characters. `Q22`'s new gate freezes these in `DECLARED_RENDER_GAPS` and asserts the set in both directions, so they cannot grow silently, but it does not close them. | Completeness of Constitution s12 condition 3's human-readable evidence | OPEN, split from `Q22` 2026-09-18 (`ADR-0097`). Closing it changes `_render`, which changes all 44 rendered evidence files and forces CI-sourced regeneration across all 22 certifiers - a different risk class, deliberately excluded from M1. |
| Q27 | Can the integrity of the repository-held trust records be anchored against anything a repository writer cannot also move (the residual of `Q24`)? | `Q24`'s remediation makes a SINGLE-file edit fail, by cross-checking `CHECKSUMS.sha256`, its digest in `ORACLE_ENVIRONMENT.json`, a pinned git blob constant, and the per-file digest duplication. A coordinated single-commit edit of all of them still passes, and `TRUST_ANCHOR_RESIDUAL` says so in the source. This is not a claim that could be strengthened by more in-repository records: the only genuine anchor is upstream provenance - `BUILD_INFO.txt` records the binary as built from `aloistr/swisseph` v2.10.03 - which needs network access the no-oracle job's guard forbids. Note also that `main` has no branch protection (`Q2`), so a history rewrite is detectable but not prevented. | Integrity of the D-001 numerical authority | OPEN, split from `Q24` 2026-09-18 (`ADR-0097`). The limitation is demonstrated, not merely described: `test_trust_record_integrity.py::test_coordinated_edit_of_every_trust_record_is_NOT_caught` asserts that such an edit passes, so it fails if the claim ever silently changes. Upstream provenance anchoring is out of scope by the owner's own M1 exclusion. |
