...
- **Certification-artifact impact, consequence of M-03 working as intended:** three non-oracle-tier artifacts (`current_engine`, `KP_CHAIN_V1`, `SIGN_CONVENTION_V1`) and, once the H-04/M-03 fixes reach the oracle tier, all eight oracle-tier artifacts, will legitimately report `preconditions.anti_fitting.modules_scanned: 170` (was 146) and a non-empty `justified_exceptions` list, because the scan surface itself changed. This is **expected drift caused directly by this entry**, not an unexplained change; `scripts/check_artifact_drift.py` correctly flags it as non-volatile, which is the drift gate performing its job, not a defect in it. Regeneration and commit of the affected artifacts is recorded in this entry's own evidence, once available from the CI oracle-tier runner (this Windows development host cannot produce it: `certify_current_engine.py` shells out to the repository's bundled `swetest`, confirmed by direct inspection to be an `ELF 64-bit LSB pie executable ... for GNU/Linux` - `[WinError 193] %1 is not a valid Win32 application` is Windows correctly refusing to execute a Linux binary, the same documented platform limitation as `ADR-0043`; the eight oracle-tier certifiers separately require PyJHora, unavailable on this host).
...
- **Evidence:** `reports/G1_ARCHITECTURE_AUDIT_2026-08-11.md` H-04 and M-03 sections; commit `37aca59` full diff; CI run `32048557295` (failure, both defects diagnosed above); direct inspection of `engine/dasha/vimshottari.py` (depth-3 default and `pratyantardashas()` pre-existing) and `engine/models/dasha.py`; manual independent invocation of `scan_for_fitting()` (170 modules, 0 findings) and of the clean-file/dirty-file negative-control pair, this session; `python -m pytest -q` - 560 passed, 0 failed, 0 skipped; `scripts/check_adr_numbering.py`, `scripts/check_identifier_families.py` - PASS; `file ./swetest` output (ELF, Linux) as the Windows-limitation diagnosis. Full CI confirmation and artifact regeneration recorded in this entry's own follow-up evidence once the corrected commit is pushed.
