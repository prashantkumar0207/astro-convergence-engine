...
- **Date:** 2026-08-27
- **Status:** PROPOSED. Not yet ratified. Recorded per this repository's own rule that a genuine owner
  decision required and not inferable from an existing ratified decision must stop and say exactly what
  is needed, rather than being guessed at or silently absorbed into other work. The repair work itself
  (below) was executed under the owner's own explicit "CEO DIRECTIVE - KP_SIGNIFICATOR_V1 CERTIFICATION
  REPAIR" instruction (2026-08-27), which is its own standing authorization for the repair steps taken;
  what remains open for ratification is the resulting disposition of the certification record and
  `DP-030`'s own separate, deeper primary-source question (see "Not decided by this entry" below).
