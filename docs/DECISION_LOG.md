...
| Field | Value |
|---|---|
| Status | ACTIVE REGISTER. **ACCEPTED (31): ADR-0001, ADR-0002, ADR-0005 through ADR-0012, ADR-0015, ADR-0016, ADR-0017, ADR-0033 through ADR-0037, ADR-0039 through ADR-0042, ADR-0044, ADR-0045, ADR-0046, ADR-0048, ADR-0049, ADR-0050, ADR-0051, ADR-0052, and ADR-0053**, owner-ratified across 2026-08-13, 2026-08-14 and 2026-08-17. Tier-0 is **FORMALLY LOCKED** (ADR-0034), the only s12 Locked artifact. The registry vargas and the sign conventions are **owner-ratified but NOT s12 Locked** (ADR-0035 D3, ADR-0036 D3). **Q8 is now CLOSED** (ADR-0048). **Phase G's B-01/B-02/B-03 blockers (ADR-0049), G7 stale documentation (ADR-0050), G2 charter adoption, G3 precedence hierarchy/agent workflow, and G5 C0-C5 certification taxonomy (all three ADR-0051, 2026-08-17) are RESOLVED.** `DP-009` is dispositioned by `ADR-0052`: rise/set Tier-0; panchanga Tier-1; vara straddles the boundary. **FOUNDATION entry criteria H-04 and M-03 are remediated by `ADR-0053`** (a genuine typo defect found and fixed during independent verification is recorded there). None of this is authorization for FOUNDATION or any product capability. **See this session's evidence report for the independently-verified Phase G exit and FOUNDATION entry-gate determinations** - this header records ratification status, not an exit or entry declaration. **Still PROPOSED: ADR-0003, ADR-0004, ADR-0013, ADR-0014, ADR-0018 through ADR-0032, ADR-0038, and ADR-0047.** Ratification is per-entry; an owner existing is not the register being ratified. Includes the ADR-0018 remote-CI evidence addendum (2026-08-11). |
| Version | 2.6.0 |
...
- **Certification-artifact impact, consequence of M-03 working as intended:** all eleven certification artifacts legitimately report `preconditions.anti_fitting.modules_scanned: 170` (was 146) and a non-empty `justified_exceptions` list once regenerated, because the scan surface itself changed; `VIMSHOTTARI_V1` additionally reports the H-04 schema change (`oracle_bhukti_rows_compared` -> `oracle_pratyantar_rows_compared`, 1782 -> 16038 rows, `oracle_max_start_delta_days` 1.86e-09 days, `oracle_lord_mismatches: 0`). This is **expected drift caused directly by this entry**, not an unexplained change; `scripts/check_artifact_drift.py` correctly flagging it as non-volatile is the drift gate performing its job, not a defect in it.
  ...
  - **`current_engine`, `KP_CHAIN_V1`, `SIGN_CONVENTION_V1` remain un-regenerated in this entry - a diagnosed, narrow, remaining gap, not silently dropped.** `current_engine` cannot run on this Windows host at all: `certify_current_engine.py` shells out to the repository's bundled `swetest`, confirmed by direct inspection (`file ./swetest`) to be an `ELF 64-bit LSB pie executable ... for GNU/Linux` - `[WinError 193] %1 is not a valid Win32 application` is Windows correctly refusing to execute a Linux binary, the same documented platform limitation as `ADR-0043`. The CI oracle job's own upload step (`path: certification/*.json`) was checked directly and confirmed to upload whatever is in that directory at upload time, not to regenerate `current_engine`/`KP_CHAIN`/`SIGN_CONVENTION` - its "Oracle certification runners" step runs exactly the eight scripts named above and no others (`.github/workflows/ci.yml` lines 233-244), so the downloaded bundle's copies of these three were confirmed (direct field comparison) to be the unregenerated, already-committed 146-scan versions, not fresh evidence, and were discarded rather than mistaken for real regeneration. `KP_CHAIN` and `SIGN_CONVENTION` **can** and were regenerated locally on this Windows host (`pyswisseph`, no `swetest`/PyJHora dependency) and produced the correct M-03 drift with zero other change - but the local run's console transcript records the archived path with a Windows backslash (`certification\KP_CHAIN_V1_certification.json`) where the committed evidence and every other artifact record a POSIX forward slash, a genuine cross-platform provenance difference this entry declines to either commit (would introduce a platform-specific artifact) or hand-edit (forbidden: certification evidence is never hand-edited). All three are left at their last-committed (146-scan) state, correctly stale, until a Linux-sourced run regenerates them; this does not affect H-04 or M-03's own correctness, both independently verified above.
