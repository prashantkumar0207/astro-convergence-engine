...
**Certified but NOT production-registered: D16 Shodasamsa (`ADR-0089`) and D4 Chaturthamsa (`ADR-0090`).** Both hold PASS certification artifacts (`certification/VARGA_D16_V1_certification.json`, `certification/VARGA_D4_V1_certification.json`), produced against standalone rules instantiated inside their own certifier scripts. Neither has a production module and neither appears in `CERTIFIED_PRODUCTION_VARGAS`, so `divisional_chart(snapshot, 16)` and `divisional_chart(snapshot, 4)` raise `UnsupportedVargaError`, correctly. **Both ARE wired into CI's oracle-tier job (`ADR-0096`)** and both hold genuine external-oracle agreement - PyJHora 4.8.7, 5,400 comparisons, 0 mismatches each. Corrected 2026-09-18: this paragraph previously said neither was wired into CI, which `ADR-0096` superseded. **CI execution is not registration**, and running a certifier in CI grants no production status. **Production implementation for both is a separate, not-yet-given authorization.** D20 followed exactly this path and has since been production-registered under `ADR-0095`. Certification is not registration, and this repository does not treat it as such.
