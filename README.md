...
VARGA REGISTRY, BATCH THREE THROUGH FIVE: D7 Saptamsa, D30 Trimsamsa, and D2 Hora (Parashara variants) CERTIFIED per `certification/VARGA_D7_V1_certification.json`, `VARGA_D30_V1_certification.json`, and `VARGA_D2_V1_certification.json` (ADR-0011, ADR-0011, ADR-0011). Same six-gate discipline per varga: dual-transcribed literals (D7's parity start table, D30's tara-graha unequal segments with a rulership re-derivation, D2's pinned two-sign Leo/Cancer output space), 51,429-point sweeps, ULP batteries, PyJHora Parasara oracles 3,600/3,600 each, non-invasive registration (D9/D10 bit-identical), independent by-name validators. *(Current state, corrected 2026-09-05 per ADR-0093: the registry now serves **eight** divisions under parashara - D2, D3, D7, D12, D24, D30, D40, D45. The three added since this paragraph was written are D45 Akshavedamsa (ADR-0077), D24 Siddhamsa (ADR-0083) and D40 Khavedamsa (ADR-0087), each with its own PASS certification artifact.)*

VARGAS CERTIFIED BUT NOT PRODUCTION-REGISTERED: D16 Shodasamsa (ADR-0089) and D4 Chaturthamsa (ADR-0090). Both hold PASS artifacts (`certification/VARGA_D16_V1_certification.json`, `certification/VARGA_D4_V1_certification.json`) produced against standalone rules defined inside their own certifier scripts. Neither has a production module, neither is in `CERTIFIED_PRODUCTION_VARGAS`, and neither is wired into CI, so `divisional_chart(snapshot, 16)` and `divisional_chart(snapshot, 4)` raise UnsupportedVargaError by design. Production implementation for both is a separate authorization that has not been given. In this repository certification is not registration.

FOUNDATION AND JATAKA CAPABILITIES CERTIFIED SINCE THE PARAGRAPHS ABOVE WERE WRITTEN: RISE_SET_V1 (ADR-0054), PANCHANGA_V1 (ADR-0055, tithi/vara/nakshatra/yoga/karana), TRIKALAM_V1 (ADR-0060, Rahu Kalam/Yamaganda/Gulika under the named seed variant PYJHORA_TRIKALAM_V1), KP_SIGNIFICATOR_V1 (ADR-0078, with the certification-integrity repair in ADR-0079), and PARASHARI_YOGA_V1 (ADR-0081, the five Panch Mahapurusha yogas only, read subject to the ADR-0086 composition-layer qualification). FOUNDATION was formally exited 2026-08-22 (ADR-0068). Each capability has its own regenerable certification artifact under `certification/`.

PARASHARI SCHOOL LAYER (`engine/parashari/`): PARASHARI_DRISHTI_V1 CERTIFIED per `certification/PARASHARI_DRISHTI_V1_certification.json` (ADR-0012), the first module of the school-separated aspect-systems architecture. Full (purna) graha drishti facts (aspected signs, whole-sign houses, planets) from certified Parashari placements: dual-transcribed offset table, exhaustive 84-pair combinatorics, PyJHora oracle over the 11-case holdout (154 comparisons, zero mismatches), independent name-counting validator. Node-cast aspects are excluded by approved decision AS-B (recorded variant); sputa drishti, Jaimini rashi drishti, Western aspects and interpretation are non-claims. *(Corrected 2026-09-05 per ADR-0093: "yogas" was removed from this non-claim list, which had become false - the five Panch Mahapurusha yogas are certified as PARASHARI_YOGA_V1, ADR-0081. No yoga beyond those five is claimed.)*
...
The ratified roadmap is **RESOLVED** as of 2026-08-17: `ADR-0048` ratified `docs/Q8_CLOSURE_MATRIX.md` as the roadmap-governance framework, closing `docs/OPEN_QUESTIONS.md` Q8. Ratification approves the framework only; it authorises entry into no phase, and each phase remains gated by its own prerequisites, entry criteria and CEO approval. *(Corrected 2026-09-05 per ADR-0093; this sentence previously read "still OPEN".)* `docs/PROJECT_BACKLOG.md` is the enumerated plan of record: Phase 1 Core Intelligence (Question, Knowledge, Inference and Prediction engines) precedes Phase 2 Astrology Systems, then Phase 3 validation against historical cases, Phase 4 API, Phase 5 frontend, Phase 6 production. The astrology layers certified so far are Phase 2 content built ahead of Phase 1; that divergence is recorded in `docs/DECISION_LOG.md` ADR-0013. `ARCHITECTURE_STATUS.md` carries the remediation-era engineering order for reference, not as a ratified sequence.
